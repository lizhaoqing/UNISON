...
class MelConverter(nn.Module):
    ...
    def forward(self, waveform: torch.Tensor, center: bool = False) -> torch.Tensor:
        ...
        # Dithering: only for 16kHz where fmax=Nyquist causes edge artifacts
        if self.sampling_rate <= 16000:
            dither_scale = 1e-4
            noise = torch.randn_like(waveform) * dither_scale
            silence_mask = (torch.abs(waveform) < dither_scale)
            waveform = torch.where(silence_mask, noise, waveform)
